...
### Step 2 — Provision the backend on Render
...
| PostgreSQL | `anga-db` | Kenya gazetteer |
| Key Value (Redis) | `anga-cache` | Weather cache, single-flight locks, quota state |

`DATABASE_URL` and `REDIS_URL` are wired automatically, and `DJANGO_SECRET_KEY` is
generated by Render — it never exists in the repository.
...
In the `anga-api` service → **Environment**, set the values marked `sync: false`:

| Key | Value |
| --- | ----- |
| `WEATHER_AI_API_KEY` | your `wai_…` key |
| `FRONTEND_ORIGINS` | leave blank for now — filled in at Step 6 |
...
- **Render free PostgreSQL expires after 90 days.** The gazetteer re-seeds itself on
  deploy, so recovering is a matter of provisioning a new database and redeploying.
